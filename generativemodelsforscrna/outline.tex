...
\[
\lambda^c_g = f_g \left(\sum_{i=0}^{\ell} h_{T^c_i}(Z^c_{T^c_i})\right)
\]
...
\item The functions $h_{T^c_i}$ indicates how these cell-type-specific features alter the distribution on the gene expressions for cells in that branch of the tree.  
\end{itemize}
